...
同じメンバー・同じ実施日のスコア申請は複数登録できます。管理者の承認は
`approve_score_request(uuid)` RPCで行い、同日のセッションの末尾へゲームと
フレームを追加します。記録と承認状態は同一トランザクションで保存し、同じ
申請の再承認では二重登録しません。ゲーム数・ゲーム番号の旧5G上限は解除済みです。
画像読み取りの1日5回制限は、申請・保存とは別の制限として継続します。

画像読み取り関数は Gemini 3.8 Flash、3.7 Flash、3.6 Flash の順で利用します。
モデル別の呼び出し回数は `gemini_model_usage` に記録し、Google側の1日20回枠を
使い切らないよう太平洋時間の1日18回で次のモデルへ切り替えます。Google APIが
先に429を返した場合も、同じ読み取り処理の中で次のモデルへ切り替えます。
