...
function updateDashboardMemberCarouselDepth() {
  ...
  container.querySelectorAll('.dashboard-member-slide').forEach((card) => {
    ...
    const stride = Math.max(1, slot.offsetWidth * 0.43);
    const offset = (slot.offsetLeft + slot.offsetWidth / 2 - center) / stride;
    ...
    slot.style.zIndex = String(Math.max(1, 1000 - Math.round(distance * 100)));
...
function setupDashboardMemberCarouselDepth() {
  ...
  if (!container.dataset.depthReady) {
    ...
    container.addEventListener('focusin', (event) => {
      ...
      if (card && card.matches(':focus-visible')) container.scrollTo({ left: card.parentElement.offsetLeft + card.parentElement.offsetWidth / 2 - container.clientWidth / 2, behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' });
    });
...
function renderDashboardMemberCarousel() {
  ...
  container.innerHTML = members.map((member) => {
    ...
    return `
      <div class="dashboard-member-slot"><button class="dashboard-member-slide member-card-${cardTier}" onclick="openMemberDashboard('${member.id}')" aria-label="${escapeHtml(member.name)}さんのダッシュボードを開く">
